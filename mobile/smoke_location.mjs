...
cancelled.abort();
const previousCalls = positionCalls;
await assert.rejects(getDecisionLocation(cancelled.signal), (error) => error.name === 'AbortError');
assert.equal(positionCalls, previousCalls);
const duringPermission = new AbortController();
location.requestForegroundPermissionsAsync = async () => { duringPermission.abort(); return { status: 'granted' }; };
await assert.rejects(getDecisionLocation(duringPermission.signal), (error) => error.name === 'AbortError');
assert.equal(positionCalls, previousCalls, 'stopping during permission must prevent subsequent GPS lookup');
