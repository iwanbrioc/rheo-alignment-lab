...
def main():
    ap = argparse.ArgumentParser(description='Blind one v0.4 shared-map rerun before scoring.')
    ap.add_argument('run_dir', help='Path to one *-FLOW-V0-4 run directory')
    args = ap.parse_args()
    ...
    for p in src_files:
        ...
        if obj.get('experiment')!='flow-v0.4-shared-map-rerun-v1':
            raise SystemExit(f'Unexpected experiment in {p.name}')
        if not obj.get('condition'):
            raise SystemExit(f'Missing condition in {p.name}')
        parsed.append((p,obj))
    ...
    key={
        'blindingVersion':'flow-v0.4-shared-blind-v1',
    ...
    freeze={
        'freezeVersion':'flow-v0.4-shared-output-source-freeze-v1',
        'createdAt':datetime.now(timezone.utc).isoformat(),
        'caseId':next(iter(case_ids)),
        't0Sha256':next(iter(t0_hashes)),
    ...
    print('Upload the blinded directory only. Keep the private key sealed until scores are frozen.')
