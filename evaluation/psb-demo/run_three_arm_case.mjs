...
if (!a.case) throw new Error('Usage: node evaluation/psb-demo/run_three_arm_case.mjs --case /path/to/T0.json --base http://localhost:8080 --samples 3');
...
const outDir = path.join(path.dirname(CASE), 'model-runs', runId);
await mkdir(outDir, { recursive: true });

const conditions = shuffled(['rheo','future_generations','control']);
const log = [];
console.log(`PSB demo three-arm | case=${t0.caseId} | T0 sha256=${hash.slice(0,12)}… | samples=${SAMPLES} | granularity=${GRAN}`);
  ...
  for (let sample = 1; sample <= SAMPLES; sample++) {
    ...
    try {
      r = await fetch(`${BASE}/api/analyze`, {
        ...
        body: JSON.stringify({ caseRecord: t0, condition, granularity: GRAN })
      });
      raw = await r.text();
      try { body = JSON.parse(raw); } catch { body = null; }
    } catch (e) {
      log.push({ condition, sample, ok:false, status:0, ms:Date.now()-started, error:e.message });
      console.log(`${condition.padEnd(18)} s${String(sample).padStart(2,'0')} FAIL transport ${e.message}`);
      continue;
    }

    const rec = {
      condition, sample, ok:r.ok, status:r.status, ms:Date.now()-started,
      provider:body?.provider || null, model:body?.model || null,
      responseId:body?.responseId || null,
      researchUsable:Boolean(body?.researchUsable),
      error:r.ok ? null : (body || raw)
    };
    log.push(rec);

    if (r.ok && body?.map) {
      const env = {
        experiment:'psb-demo-three-arm-v1',
        caseId:t0.caseId,
        t0Sha256:hash,
        condition,
        sample,
        ...
        researchUsable:body.researchUsable,
        map:body.map
      };
      await writeFile(
        path.join(outDir, `${t0.caseId}.${condition}.s${String(sample).padStart(2,'0')}.json`),
        JSON.stringify(env, null, 2)
      );
    }
    console.log(`${condition.padEnd(18)} s${String(sample).padStart(2,'0')} ${r.ok ? 'ok  ' : 'FAIL'} ${r.status} ${rec.ms}ms`);
  }
}
...
const meta = {
  ...
  granularity:GRAN,
  conditionExecutionOrder:conditions,
  log
};
