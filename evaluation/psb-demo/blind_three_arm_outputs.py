...
ALLOWED_CONDITIONS = {'rheo', 'future_generations', 'control'}
...
def main():
    ap = argparse.ArgumentParser(description='Blind frozen three-arm PSB demo outputs before scoring.')
    ...
    for p in src_files:
        ...
        if obj.get('condition') not in ALLOWED_CONDITIONS:
    ...
    conditions = {o.get('condition') for _, o in parsed}
    ...
    if conditions != ALLOWED_CONDITIONS:
        raise SystemExit(f'Run does not contain all three conditions: found {sorted(conditions)}')

    stamp = datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    blind_dir = run_dir.parent / f'{run_dir.name}-BLINDED-3ARM-{stamp}'
    blind_dir.mkdir(parents=True, exist_ok=False)
    key_path = run_dir.parent / f'{run_dir.name}-BLINDING-KEY-PRIVATE-3ARM-{stamp}.json'

    tokens = [f'M{i:02d}' for i in range(1, len(parsed) + 1)]
    secrets.SystemRandom().shuffle(tokens)

    key = {
        'blindingVersion': 'psb-demo-three-arm-blind-v1',
    ...
    freeze = {
        'freezeVersion': 'psb-demo-three-arm-source-freeze-v1',
        'createdAt': datetime.now(timezone.utc).isoformat(),
        'caseId': next(iter(case_ids)),
        't0Sha256': next(iter(t0_hashes)),
