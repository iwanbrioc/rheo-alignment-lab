...
if len(sys.argv) != 2:
    raise SystemExit('Usage: python3 evaluation/synthetic-action-v0.8/blind_outputs.py /path/to/model-runs/<run-id>')
...
if run_log.get('experiment') != 'synthetic-action-v0.8':
    raise SystemExit('Run log is not synthetic-action-v0.8.')
...
for p in sorted(src.glob('SYN-*.json')):
    ...
    if d.get('experiment') == 'synthetic-action-v0.8':
        records.append((p, d))

if not records:
    raise SystemExit('No synthetic-action-v0.8 outputs found.')

samples = int(run_log.get('samplesPerCondition', 3))
conditions = tuple(run_log.get('design', {}).get('conditions', ['bare','matched','rheo']))
...
for case_id in case_ids:
    for condition in conditions:
        if counts[(case_id, condition)] != samples:
            raise SystemExit(f'Unbalanced run for {case_id}/{condition}: {counts[(case_id, condition)]} outputs, expected {samples}.')
...
out.mkdir()
key = {'experiment':'synthetic-action-v0.8','sourceRun':str(src),'seed':seed,'items':[]}
...
(out/'README.txt').write_text(
    'Blinded v0.8 outputs. Files contain only case identity and the ontology-neutral comparison object.\n'
    'There are three hidden generation conditions. Do not infer or guess condition identity.\n'
    'Do not inspect the private key until all ratings/comparisons are frozen.\n'
)
